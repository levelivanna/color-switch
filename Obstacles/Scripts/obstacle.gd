class_name Obstacle
extends Node2D
@export var min_rotation_speed: float = 70.0 
@export var max_rotation_speed: float = 90.0
...
func increase_rotation():
	var current_score = GLOBALS.get_score()
	if(current_score < GLOBALS.increase_speed_score):
		return
	rotation_speed = rotation_speed * (1 + current_score / 100)
